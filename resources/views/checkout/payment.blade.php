...
<main class="max-w-3xl mx-auto px-6 py-20 text-center">
    <div class="bg-white rounded-3xl border border-slate-200 p-12 shadow-sm inline-block w-full max-w-md">
        ...
        {{-- KOTAK PERINGATAN (Akan muncul jika user meng-close VA tapi belum bayar) --}}
        @if(session('warning'))
            <div class="mb-6 p-4 bg-amber-50 text-amber-700 border border-amber-200 rounded-xl text-sm font-bold text-left flex items-start gap-3">
    ...
    document.getElementById('pay-button').onclick = function () {
        snap.pay('{{ $transaction->snap_token }}', {
            ...
            onPending: function(result){
                // Ini akan melempar ke Controller, tapi Controller sekarang akan menendang balik
                // ke halaman ini karena statusnya masih pending.
                window.location.href = "{{ route('checkout.success', $transaction->order_id) }}";
            },
            onError: function(result){
                window.location.href = "{{ route('checkout.cancel', $transaction->id) }}";
            },
            onClose: function(){
                // Tidak melakukan redirect, biarkan user tetap di halaman pembayaran
