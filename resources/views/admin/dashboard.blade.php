...
<div class="flex justify-between items-center mb-10">
    <div>
        <h1 class="text-3xl font-black text-gray-900">Dashboard Ringkasan</h1>
...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
    ...
    <div class="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
        ...
        <h3 class="text-2xl font-black">{{ $tiketTerjual ?? 0 }}</h3>
    </div>
        ...
        <table class="w-full text-left">
            ...
            <tbody class="divide-y border-t">
                ...
                <tr class="hover:bg-slate-50 transition">
                    <td class="px-8 py-6">
                        {{-- Logika baru: mencari kolom name, customer_name, atau buyer_name secara otomatis --}}
                        <p class="font-bold uppercase tracking-wide text-sm">{{ $trx->name ?? $trx->customer_name ?? $trx->buyer_name ?? 'Tanpa Nama' }}</p>
                        <p class="text-xs text-slate-400">{{ $trx->email ?? $trx->customer_email ?? '-' }}</p>
                    </td>
                    <td class="px-8 py-6 font-medium text-slate-600">{{ $trx->event->name ?? '-' }}</td>
                    <td class="px-8 py-6">
                ...
                @empty
                <tr>
                    <td colspan="4" class="px-8 py-6 text-center text-slate-400">Belum ada transaksi.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
